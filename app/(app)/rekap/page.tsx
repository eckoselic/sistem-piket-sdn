...
export default function RekapPage() {
  ...
  const muatData = useCallback(async () => {
    ...
    const [{ data: jadwal }, { data: presensi }, { data: jurnal }] = await Promise.all([
      supabase.from("jadwal_piket").select("guru_id, profiles!guru_id(nama)").gte("tanggal", rentang.awal).lte("tanggal", rentang.akhir),
      supabase.from("presensi").select("guru_id, status").gte("tanggal", rentang.awal).lte("tanggal", rentang.akhir),
      supabase.from("jurnal_kejadian").select("*, profiles(nama)").gte("tanggal", rentang.awal).lte("tanggal", rentang.akhir).order("tanggal", { ascending: true }),
    ]);
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <button onClick={handleExportPdf} disabled={membuatPdf || loading} className="rounded-card bg-ink-light px-4 py-2 text-sm font-semibold text-paper hover:bg-ink disabled:opacity-60">
      ...
      <div className="p-6 md:p-10 space-y-6">
        <div className="flex gap-3">
          <select value={bulan} onChange={(e) => setBulan(Number(e.target.value))} className="rounded-card border border-ink/15 px-3 py-2 text-sm outline-none focus:border-ink-light">
            {BULAN.map((b, i) => (
          ...
          <select value={tahun} onChange={(e) => setTahun(Number(e.target.value))} className="rounded-card border border-ink/15 px-3 py-2 text-sm outline-none focus:border-ink-light">
        ...
        <div className="rounded-card border border-ink/8 bg-white overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-4 py-6 text-center text-slate-soft">
                    Memuat…
                  </td>
                </tr>
              ) : ringkasan.length === 0 ? (
        ...
        <div className="rounded-card border border-ink/8 bg-white p-5">
          ...
          <p className="mt-1 text-xs text-slate-soft">Seluruh catatan ini akan disertakan pada lampiran PDF rekap.</p>
        </div>
      </div>
    </div>
  );
